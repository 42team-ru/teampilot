...
@router.message(
    StateFilter(RegistrationStates.waiting_for_first_name, RegistrationStates.waiting_for_last_name),
    Command("cancel"),
)
async def cancel_registration(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer(
        "Регистрация отменена. Чтобы пользоваться ботом, отправьте /start и введите имя и фамилию."
    )
...
async def process_last_name(message: Message, state: FSMContext) -> None:
    ...
    if result is None:
        await message.answer("Не удалось сохранить данные. Попробуйте позже.")
        return
    ...
    user_info = {
        ...
        "systemRole": result.get("systemRole", "USER"),
    }
...
async def require_registration(
    ...
    if lookup_failed:
        await message.answer("Не могу проверить регистрацию: backend недоступен. Попробуйте позже.")
        return

    await state.update_data(
        registration_pending_command=message.text,
        registration_user_id=existing_user.get("userId") if existing_user else None,
    )
    await state.set_state(RegistrationStates.waiting_for_first_name)
    await message.answer(
        "Сначала нужно представиться. Введите ваше имя. Например: <b>Иван</b>",
        parse_mode="HTML",
    )
...
async def require_registration_in_group(message: Message, lookup_failed: bool = False) -> None:
    ...
    await message.answer(
        "Сначала зарегистрируйтесь в личке бота: введите имя и фамилию, потом повторите команду.",
...
async def _continue_after_registration(
    ...
    if manager_teams:
        ...
        name = ""
        if resolved_user:
            name = f", {resolved_user.get('firstName', '')}"
        await message.answer(
            f"Привет{name}! Вы успешно зарегистрированы. "
            "Чтобы начать работу, попросите менеджера добавить вас в команду."
        )
