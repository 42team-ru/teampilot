...
async def cmd_upload(message: Message, state: FSMContext) -> None:
    ...
    if not all_teams:
        await message.answer(
            ...
            "Используйте кнопку 📤 в контексте нужной команды через меню 🏢 Выбрать команду."
        )
        return
    ...
    await message.answer(
        ...
        "🏢 Выбрать команду → выберите команду → 📤 Загрузить файл"
    )
...
async def handle_upload_file(
    ...
    if team_chat_id is None:
        await message.answer(
            ...
            "Используйте кнопку 📤 в контексте команды через меню 🏢 Выбрать команду."
        )
        await state.clear()
        return
